...
{  
    ...
    int number ,left,right,digit;
    for ( int i = 0; i<n ;i++)
    {
        number = a[i];
        while(number!=0)
        {
        single_number.push_back(number%10);
        number = number/10;
        }
        left=0;
        right = single_number.size() -1;
        
        while(left<right)
        {
            if(single_number[left] == single_number[right])
            {
               left++;
               right--;
            }
        ...
        single_number .erase(single_number.begin(),single_number.end());
